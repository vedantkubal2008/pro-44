...
function setup() {  
  ...
  space.velocityY = (50 + 10* score/10);
  ...
  endline = createSprite(350,700,1200,25);
...
function draw() {
  ...
  if(gameState === play) {
   ...
   if(space.y > 800) {
    ...
    text("LaserUsed : " + shootc,610,60)
    text("/50  " ,830,60)
    if(asteroidGroup.isTouching(endline) || shootc>=50) {
      asteroidGroup.destroyEach();
      gameState = end;
    }
  ...
  if(gameState === instruction) {
    ...
    text("  Some asteroids are coming towards the Earth !",canvas.width/2-300, canvas.height/2 - 215);
    text("  You are a space fighter.",canvas.width/2-300,canvas.height/2-180);
    text("  Save the people and the Earth !",canvas.width/2-300,canvas.height/2-145);
    text("  Press the 'spacebar' to shoot.",canvas.width/2-300,canvas.height/2-110);
    text("  Use the right and left arrows to move.",canvas.width/2-300,canvas.height/2-75);
    text("  Don't let the asteroid be missed.",canvas.width/2-300,canvas.height/2-40);
    text("  You have only 50 lazer bullets.",canvas.width/2-300,canvas.height/2-5);
    text("  Press 's' to start the game.",canvas.width/2-300,canvas.height/2+40);
...
function asteroids() {
  if(frameCount % 110 === 0) {
    ...
    var asteroid = createSprite(Math.round(random(100,650)),-20);
